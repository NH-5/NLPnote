...
一种常见类型是交集型歧义.  
如果汉字串 `AJB` 中, `AJ` 和 `JB` 都是词, 则 `AJB` 可形成交集型歧义.
...
另一种常见类型是组合型歧义.  
如果汉字串 `AB` 中, `A`、`B`、`AB` 都是词, 则 `AB` 可形成组合型歧义.
...
- 新词, 如“博客”“房奴”“内卷”等.
- 专有名词, 如人名、地名、机构名、时间、数字等.
- 专业名词, 如医学、金融、工程等领域中的术语.
...
$
  w^* = "arg max"_(w in Omega(s)) P(w | s)
$

因为原始字符串$s$由切分结果$w$连接而成, 常把问题转化为最大化切分序列本身的概率:
$
  w^* = "arg max"_(w in Omega(s)) P(w)
$

若采用 n-gram 语言模型, 可以近似为:
$
  P(w) approx product_(i=1)^m P(w_i | w_(i-1))
$
...
- 单字上下文: $C_k$, 其中$k=-2,-1,0,1,2$.
- 相邻字组合: $C_k C_(k+1)$, 其中$k=-2,-1,0,1$.
- 跨字组合: $C_(-1) C_1$.
- 字符类别特征: 数字、标点、英文字母等.
...
- 跨领域应用时性能不理想, 例如用新闻语料训练的模型处理医学文本.
- 面对非规范文本时性能下降, 例如微博、短信、网络评论等.
- 新词、缩写、口语表达和专有名词不断出现, 需要模型具备持续适应能力.
...
- 几乎总是用作姓氏的字, 如陈、丁、邓、蒋等.
- 偶然用作姓氏的字, 如曾、都、向、于等.
- 作姓氏与非姓氏机会接近的字, 如马、黄、张等.
- 待定字, 如任、房、方等.

上下文规则则利用周围词语判断一个字是否更可能作为姓名的一部分.  
例如:

- “老/小 + 姓氏用字”通常可识别人称, 如“小王”“老李”.
- “姓氏用字 + 工/总”通常可识别人称, 如“张工”“陈总”.
- “数词 + 可作量词的姓氏用字”通常不识别人名, 如“一周”“七章”.
...
1. 将句子中的字或词映射为 embedding.
2. 将 embedding 序列输入神经网络.
3. 由神经网络提取上下文特征.
4. 通过分类层或 CRF 层预测每个位置的实体标签.
...
现代 NER 系统常采用 `B-I-O` 或 `B-M-E-S` 标注体系.  
例如 `B-PER` 表示人名开始, `I-PER` 表示人名内部, `O` 表示实体外部.
...
- LDC 的 CTB 语料库将汉语词性划分为 33 类.
- 北大语料库将汉语词性划分为 26 类一级词类.
- 其他研究也可能采用 25 类或其他粒度的划分.
...
- Brown 标注集, 共 86 个标记.
- Penn Treebank 标注集, 共 45 个标记.
- CLAWS C5 标注集, 共 62 个标记.

常见中文标注集包括:

- LDC CTB 标注集, 共 33 个标记.
- 北大标注集, 包含 26 个一级词类标记和 106 个全部标记.
- 中科院 ICTCLAS 词性标注集, 共 39 个标记.
...
1. 使用初始标注器得到一份自动标注结果.
2. 将自动标注结果与人工标准答案比较.
3. 根据预先设计的规则模板提取候选转换规则.
4. 选择使错误数减少最多的规则加入规则库.
5. 用规则库修正当前标注结果.
6. 若还能找到有效规则, 则继续迭代; 否则终止.
...
例如:
$
  P(T) approx product_(i=1)^T P(t_i | t_(i-1))
$
$
  P(W | T) approx product_(i=1)^T P(w_i | t_i)
$

于是目标变为:
$
  T^*
  = "arg max"_(t_1, dots, t_T)
    product_(i=1)^T P(t_i | t_(i-1)) P(w_i | t_i)
$
...
- 如何设计模型框架: 明确状态、观测、转移概率和发射概率.
- 如何训练模型: 从已标注语料中估计转移概率和发射概率.
- 如何进行标注: 用 Viterbi 算法寻找最优词性序列.
...
- 在新闻、百科等规范文本上, 分词和词性标注已经比较成熟.
- 在人名、地名、机构名等常见实体上, NER 也能取得较好效果.
- 在跨领域、口语化、网络化和新实体密集场景中, 仍然存在明显挑战.
...
2. 简述命名实体识别的概念.
3. 列举北大版词性标注集的基本词性代码.
4. 简述如何利用 HMM 建立词性标注器.
5. 分别用正向最大匹配法和反向最大匹配法对给定句子进行分词.

这些问题对应本讲的核心线索:

- 分词如何从词典方法发展到序列标注方法.
- NER 如何识别文本中的命名性实体指称.
- POS tagset 如何限定词性标注的类别空间.
- HMM 如何把词性标注建模为“观测词序列 -> 隐藏词性序列”的解码问题.
