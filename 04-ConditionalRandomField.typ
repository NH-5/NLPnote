...
$
  P(Y) = 1/Z product_C psi_C (Y_C)
$
其中:

- $C$表示图中的团.
- $Y_C$表示团$C$上对应的随机变量.
- $psi_C(Y_C)$是定义在团上的非负函数, 称为势函数.
- $Z$是规范化因子(partition function), 保证概率和为$1$.

规范化因子写作:
$
  Z = sum_Y product_C psi_C (Y_C)
$
...
$
  P(Y_v | X, Y_(W minus v)) = P(Y_v | X, Y_(N_v))
$
则称$P(Y|X)$是条件随机场.
...
$
  P(y|x) = 1/(Z(x)) product_C Psi_C (y_C, x)
$
其中
$
  Z(x) = sum_y product_C Psi_C (y_C, x)
$
...
- 每个结点$y_i$表示位置$i$上的标签.
- 边只连接相邻标签$y_(i-1)$和$y_i$.
- 输入$x$作为全局已知条件.

因此, 线性链CRF的条件概率可写为:
$
  P(y|x) = 1/(Z(x)) product_(i=1)^n Psi_i (y_(i-1), y_i, x, i)
$
...
- $f_k(y_(i-1), y_i, x, i)$是特征函数(feature function).
- $omega_k$是对应的权重参数.
...
- 转移特征(transition feature): 描述相邻标签之间的关系.
- 状态特征(state feature): 描述当前位置标签与输入之间的关系.
...
- 转移特征可以表示“前一个词性是名词, 当前词性是动词”.
- 状态特征可以表示“当前词是`的`, 当前标签是助词”.
...
则有:
$
  Z(x) = sum_y product_(i=1)^n M_i (y_(i-1), y_i | x)
$

记标签集合大小为$m$, 并记
$
  alpha_i = vec(alpha_i(y_1), alpha_i(y_2), dots.v, alpha_i(y_m))
$
$
  beta_i = vec(beta_i(y_1), beta_i(y_2), dots.v, beta_i(y_m))
$

如果把起始状态$y_0$吸收到第一个位置的势函数中, 则可以把
$
  alpha_1 = vec(M_1(y_0, y_1 | x), M_1(y_0, y_2 | x), dots.v, M_1(y_0, y_m | x))
$
看成初始前向列向量.

之后的递推与 HMM 的矩阵形式非常相似:

- 前向递推通过$M_i^T$左乘前向列向量.
- 后向递推通过$M_(i+1)$左乘后向列向量.
- 规范化因子由最终前向量求和得到.
...
- 概率计算: 计算给定$x$和$y$下的概率、边缘概率及特征期望.
- 推测状态: 求使$P(y|x)$最大的标签序列.
- 模型学习: 用训练数据估计参数$omega$.
...
$
  alpha_i (y_i)
    = sum_(y_1, dots, y_(i-1))
      product_(j=1)^i M_j (y_(j-1), y_j | x)
$
...
$
  beta_i (y_i)
    = sum_(y_(i+1), dots, y_n)
      product_(j=i+1)^n M_j (y_(j-1), y_j | x)
$
...
把所有相邻状态边缘概率写成矩阵, 记为:
$
  P_i = 1/(Z(x)) "diag"[alpha_(i-1)] M_i "diag"[beta_i]
$
...
写成列向量形式:
$
  delta_i = vec(delta_i(y_1), delta_i(y_2), dots.v, delta_i(y_m))
$
...
#figure(
  table(
    ...
    [建模对象], [$P(x, y)$], [$P(y|x)$],
    [模型类型], [生成式模型], [判别式模型],
    [观测假设], [当前观测只依赖当前状态], [可使用更丰富的输入特征],
    [特征表示], [通常较固定], [可灵活设计重叠特征],
    [归一化方式], [局部概率归一化], [全局条件概率归一化],
    [适用任务], [生成与识别都可], [更适合监督式标注任务]
  )
)

两者的主要区别可以概括为:

- HMM需要对观测序列的生成过程做更强假设.
- CRF直接对条件概率建模, 能更灵活地融合上下文特征.
- 在判别任务中, CRF通常比HMM具有更强的表达能力.

但与此同时:

- HMM结构更简单, 推导更直观.
- CRF训练代价更高, 需要反复计算规范化因子和特征期望.
